...
class PurchaseService
  ...
  def call
    ...
    if !booking_response
      return {error: "Такого бронирования не существует", status: 406, result: false}
    else
    ...
    if get_response_find
      return {error: "Данный пользователь уже имеет билет", status: 406, result: false}
    end
    if !normal_age?
      return {error: "Билеты доступны только с 13 лет", status: 406, result: false}
    end
    sidekiq_response = delete_response_sidekiq
    if sidekiq_response != 200
      return {error: "Problems with sidekiq", status: sidekiq_response, result: false}
    end
    put_tickets_response = put_tickets_status
    if put_tickets_response != 200
      return {error: "Problems with tickets service", status: put_tickets_response, result: false}
    end
    post_user_response = post_user
    if post_user_response != 204
      return {error: "Problems with User service", status: post_user_response, result: false}
    end
