...
# GET /tickets/price?event_date=2024-05-01T20:00:00
def price
  event_datetime = DateTime.parse(params[:event_date])
  # Находим цену для Fan зоны
  free_tickets_count_fan = Ticket.all.where("category = ? and event_date = ? and status = ?", 'fan', event_datetime, 'free').count
  fan_price = TicketsService.calculate_ticket_cost(free_tickets_count_fan, 'fan')
  # Находим цену для Vip зоны
  free_tickets_count_vip = Ticket.all.where("category = ? and event_date = ? and status = ?", 'vip', event_datetime, 'free').count
  vip_price = TicketsService.calculate_ticket_cost(free_tickets_count_vip, 'vip')
  ...
  render json: [
    {
      ...
      category: 'Фан-зона',
      price: fan_price,
      event_datetime: params[:event_date]
    },
    {
      id: 2,
      category: 'VIP-зона',
 ...
 # PUT /tickets/block?ticket_number=1&document_number=123456
  ...
  def free
    event_datetime = DateTime.parse(params[:event_date])
    free_tickets_count = Ticket.all.where("category = ? and event_date = ? and status = ?", params[:category], event_datetime, 'free').count
    if free_tickets_count.zero?
      render json: 'No available tickets', status: :not_found
    else
      ticket_number = Ticket.all.where("category = ? and event_date = ? and status = ?", params[:category], event_datetime, 'free').first.ticket_number
